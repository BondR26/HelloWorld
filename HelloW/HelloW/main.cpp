...
void DecryptAes(std::vector<unsigned char>& plainText, const std::vector<unsigned char>& chipherText)
{
    ...
    if (!EVP_DecryptUpdate(ctx, &plainTextBuf[0], &plainTextSize, chipherText.data(), chipherText.size())) {
        EVP_CIPHER_CTX_free(ctx);
        throw std::runtime_error("Decrypt error");
    }

    int lastPartLen = plainTextSize;
    if (!EVP_DecryptFinal_ex(ctx, plainTextBuf.data() + plainTextSize, &plainTextSize))
    {
        EVP_CIPHER_CTX_free(ctx);
...
void Decrypt()
{
    std::vector<unsigned char> chiperText;
    ReadFile("chipher_text.txt", chiperText);
    ...
    try
    {
        DecryptAes(plainText, chiperText);
    }
    ...
    WriteFile("plain_text.txt", plainText);

    CalculateHash(plainText, hash);

    AppendToFile("plain_text.txt", hash);
}
